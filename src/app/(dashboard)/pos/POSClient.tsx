...
export default function POSClient({
  ...
  function fillRemainingForField(key: SplitPaymentKey) {
    if (remainingAmount <= 0) return;
    setSplitPaymentAmounts((prev) => ({
      ...prev,
      [key]: remainingAmount.toFixed(2),
    }));
  }

  function clearSplitPaymentFields() {
          ...
          {splitPaymentEnabled ? (
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-medium text-brown">حقول الدفع المختلط</span>
                <button
                  type="button"
                  onClick={clearSplitPaymentFields}
                  className="rounded-full border border-border bg-white px-2.5 py-1 text-[11px] font-medium text-muted transition hover:border-gold hover:text-gold"
                >
                  إفراغ الحقول
                </button>
              </div>
              ...
              {SPLIT_PAYMENT_FIELDS.map((field) => (
                <div key={field.value} className="space-y-1.5">
                  <div className="flex items-center justify-between gap-2">
                    <label className="text-sm text-brown">{field.label}</label>
                    <button
                      type="button"
                      onClick={() => fillRemainingForField(field.value)}
                      disabled={remainingAmount <= 0}
                      className="rounded-full border border-gold/30 bg-gold/5 px-2 py-0.5 text-[10px] font-medium text-gold transition enabled:hover:bg-gold enabled:hover:text-primary-foreground disabled:cursor-not-allowed disabled:opacity-40"
                    >
                      تعبئة المتبقي
                    </button>
                  </div>
